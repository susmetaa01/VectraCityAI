...
from .sub_agents import ds_agent, db_agent, db_operations_agent
...
async def call_db_operations_agent(
        request: str,
        tool_context: ToolContext,
):
    """Tool to call database operations (INSERT/UPDATE/DELETE) agent."""
    print(
        "\n call_db_operations_agent.use_database:"
        f' {tool_context.state["all_db_settings"]["use_database"]}'
    )

    agent_tool = AgentTool(agent=db_operations_agent)

    db_ops_agent_output = await agent_tool.run_async(
        args={"request": request}, tool_context=tool_context
    )
    tool_context.state["db_operations_agent_output"] = db_ops_agent_output
    return db_ops_agent_output
