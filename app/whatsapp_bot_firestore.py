...
def get_session_from_firestore(from_number):
    ...
    doc_ref = firestore_db.collection(FIRESTORE_COLLECTION_SESSIONS).document(from_number.replace('whatsapp:', ''))
    doc = doc_ref.get()
    if doc.exists:
    ...
    if incoming_type == 'location':
        ...
        if session_data:
            ...
            try:
                ...
                print(f"Warning: Could not parse waiting_since timestamp: {waiting_since_str}. Assuming recent.")
            ...
            if current_time - waiting_since <= timedelta(minutes=LOCATION_TIMEOUT_MINUTES):
                ...
                if original_payload.get('message_type') == 'media_pending_location': # Check for pending media type
                    media_gcs_uri_temp = session_data.get('media_gcs_uri_temp')
                    media_content_type_from_session = session_data.get('media_content_type_from_session')
                    ...
                    if media_gcs_uri_temp and media_content_type_from_session:
                        try:
                            ...
                            original_payload['media_gcs_uri'] = media_gcs_uri_temp # Set the final GCS URI
                            ...
                            print(f"  Media GCS URI resolved for publishing: {original_payload['media_gcs_uri']}")

                        except Exception as e:
                            print(f"  Error resolving media GCS URI from session: {e}")
                            original_payload['error'] = original_payload.get('error', '') + f"Failed to resolve media GCS URI from session: {e}"
    ...
    # --- Handle New Text/Image/Video Messages (Initiating a new request for location) ---
        ...
        if session_data:
            ...
            try:
                ...
                print(f"Warning: Could not parse waiting_since timestamp from existing session: {waiting_since_str}. Assuming recent for timeout check.")
        ...
        if incoming_type == 'media':
            ...
            if media_url and media_content_type_temp:
                ...
                try:
                    ...
                    bucket = storage_client.bucket(GCS_MEDIA_BUCKET) # Using the same media bucket, but different prefix
                    ...
                    payload_for_pubsub['message_type'] = 'media_pending_location' # Mark as pending media
                    ...
                    payload_for_pubsub['message_type'] = 'text' if message_body else 'unknown' # Revert type if media fails
                except Exception as e:
        ...
        session_data_to_store = {
            ...
            'media_content_type_from_session': media_content_type_temp # Content type for temp media
        }
...
if __name__ == '__main__':
    ...
    print(f"NOTE: For production, you'll need a separate Cloud Function or scheduled job to clean up timed-out sessions in Firestore based on 'waiting_since' timestamp.")
